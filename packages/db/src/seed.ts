...
// invoice. Enough to render a non-empty dashboard. Idempotent: skips if an org
// named "Demo HVAC" already exists, and runs every insert in one transaction so
// a mid-seed failure rolls back instead of leaving a half-seeded DB that the
// skip guard would then permanently lock in.
...
async function main() {
  ...
  const { org, alice, bob } = await db.transaction(async (tx) => {
